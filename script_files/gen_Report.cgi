#!/router/bin/perl
# This script generates a status report based on user's attribute and date selections
...
foreach (@buffer) {
   ...
   if (($_ ne "\n") && ($counter != $num_of_elems)) {   # check to know when status entry set begins and ends 
      $holder .= $_ . "<br/>";     # holder contains user status entry 
   }
   else {
      if ($counter == $num_of_elems) {$holder .= $_ . "<br/>";}    # ensure last line in report is included
...
# process and convert to total days if user selects to specify a date range for the report
if (!$all_dates) {
   ...
   while (($key, $value) = each %entries) {
	 ...
	 $rpt_days = Date_to_Days($status_date[2], $month, $status_date[1]); # convert to days from status report
...
# textfile
my $myfile = "statusReport_$remote_user.txt";

# open file for write
open (TXTFILE, "> $myfile") or die ("Cannot open statusReport.txt for write: $!"); 

# Place tags at the beginning of status report
if ($all_dates) {
   $output = "<b>TAGS:</b>  <span style='color:red;'>*All Dates*,</span>" . "<span style='color:green;'> $user, </span>" . "<span style='color:blue;'>$attr </span>" ."\n<br/><br/>";
}
else {  # all_dates is 0 because a date range is specified
   $output = "<b>TAGS:</b>  <span style='color:red;'>$from_d <= x <= $to_d,</span>" . "<span style='color:green;'> $user, </span>"  . "<span style='color:blue;'>$attr </span>" . "\n<br/><br/>";
}

# sort hash keys in descending order (newest to oldest entries) and convert hash values into string
foreach $key (sort {$b <=> $a} keys %entries) {
   $output .= $entries{$key} . "<br/>";
   #print "$key\n";
}
#print "$output";
